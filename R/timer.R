...
get_immune_curated_data_path <- function() {
  system.file("extdata", "timer", "precalculated",
    "immune.expression.curated.RData",package = "EZGen", mustWork = TRUE)
}
...
LoadImmuneGeneExpression <- function() {
  ...
  exp <- get(load(system.file("extdata", "timer", "immune_datasets", "HPCTimmune.Rdata",package = "EZGen")))
